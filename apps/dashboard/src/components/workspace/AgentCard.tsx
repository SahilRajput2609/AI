...
export interface AgentCardProps {
  ...
  status: 'online' | 'offline' | 'busy' | 'idle' | 'running'
...
export function AgentCard({
  ...
  const displayName = title || name || 'Agent'
  const displayRole = subtitle || role || status || 'Ready'
  const displayIcon = Icon || Brain
  const IconComponent = displayIcon as any
  ...
  return (
    ...
    >
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-lg bg-[#7C6BFF]/20 border border-[#7C6BFF]/30 flex items-center justify-center">
              <IconComponent size={20} className="text-[#7C6BFF]" />
            </div>
            <div>
      ...
      {capabilities && capabilities.length > 0 && (
        <div>
          <p className="text-xs text-[#6B7280] mb-2 uppercase tracking-wider">Capabilities</p>
          <div className="flex flex-wrap gap-2">
            ...
            {capabilities.length > 2 && (
              <span className="px-2 py-1 text-xs text-[#6B7280]">+{capabilities.length - 2} more</span>
            )}
          </div>
        </div>
      )}
