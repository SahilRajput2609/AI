...
const inputVariant = (i: number) => ({
  ...
  visible: {
    opacity: 1,
    y: 0,
    transition: { delay: 0.45 + i * 0.07, duration: 0.3, ease: 'easeOut' as const },
  },
})
  ...
  const openOAuthPopup = async (provider: 'github' | 'google' | 'microsoft') => {
    ...
    try {
      ...
      if (provider === 'github') {
        ...
        url = `https://github.com/login/oauth/authorize?client_id=${config.github.clientId}&scope=user:email&state=github&redirect_uri=${encodeURIComponent(redirectUri)}`
        ...
        url = `https://accounts.google.com/o/oauth2/auth?client_id=${config.google.clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=openid%20email%20profile&state=google`
      } else {
        ...
        >
          ...
          <div className="bg-[#111111] border border-[#202020] rounded-[22px] p-10 sm:p-12 shadow-[0_0_80px_rgba(0,0,0,0.5)]">
                ...
                >
                  <label className="flex items-center gap-2.5 cursor-pointer group">
                    <div className="relative">
                      ...
                      <div className="w-4 h-4 rounded border border-[#333] bg-[#090909] peer-checked:bg-[#7C6BFF] peer-checked:border-[#7C6BFF] transition-all duration-200 flex items-center justify-center">
                        <Check size={10} className="text-white opacity-0 peer-checked:opacity-100 transition-opacity" />
                      </div>
                    </div>
                    <span className="text-xs text-[#6B7280] group-hover:text-[#A1A1AA] transition-colors">Remember me</span>
                  </label>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="text-xs text-[#7C6BFF] hover:text-[#9B8FFF] transition-colors"
                  >
                    Forgot password?
                  </button>
                </motion.div>
            ...
            <motion.div variants={inputVariant(4)} initial="hidden" animate="visible" className="flex items-center gap-4 my-7">
              ...
              <span className="text-xs text-[#6B7280]">Continue with</span>
              <div className="flex-1 h-px bg-[#202020]" />
            </motion.div>
          ...
          <motion.p
            ...
            className="text-center text-xs text-[#6B7280] mt-7"
          >
            {isSignUp ? 'Already have an account?' : "Don't have an account?"}{' '}
            <button
              onClick={() => { setIsSignUp(!isSignUp); setError('') }}
              className="text-[#7C6BFF] hover:text-[#9B8FFF] hover:underline transition-all duration-200"
            >
