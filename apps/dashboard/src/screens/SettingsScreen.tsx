...
function Section({ title, description, children }: { title: string; description?: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="rounded-lg border border-[#202020] bg-[#0F0F0F] divide-y divide-[#202020]">
        {children}
      </div>
    </div>
  )
    ...
    <Section title="General" description="Workspace preferences and defaults">
      <SettingRow label="Default Project Name" description="Default project for new tasks">
        <input
          ...
          value={projectName}
          onChange={(e) => { setProjectName(e.target.value); saveSetting('projectName', e.target.value) }}
        />
      </SettingRow>
      <SettingRow label="Auto-save" description="Automatically save changes">
        <input
          type="checkbox"
          checked={autoSave}
          onChange={(e) => { setAutoSave(e.target.checked); saveSetting('autoSave', e.target.checked) }}
          ...
          <div>
            ...
            <div className="flex gap-1.5 mt-1.5">
              {m.capabilities.map((c) => (
                <span key={c} className="text-[10px] text-[#6E6E6E] bg-[#080808] border border-[#202020] px-1.5 py-0.5 rounded">{c}</span>
              ))}
            </div>
          </div>
          <div className="flex gap-2">
            <button className="h-8 px-3 rounded-lg border border-[#202020] bg-transparent text-xs text-[#A8A8A8] hover:bg-[#080808] hover:border-[#333] transition-all">Configure</button>
          </div>
        </div>
      ))}
    </Section>
    ...
    <Section title="API Keys" description="Manage API keys for AI providers">
      {['OpenAI', 'Anthropic', 'Google'].map((provider) => (
        <SettingRow key={provider} label={provider} description="••••••••••••••••">
          <button className="h-8 px-3 rounded-lg border border-[#202020] bg-transparent text-xs text-[#A8A8A8] hover:bg-[#080808] hover:border-[#333] transition-all">Update</button>
        </SettingRow>
      ))}
    </Section>
  )
  ...
  return (
    <Section title="Notifications" description="Configure notification preferences">
      <SettingRow label="Task Updates" description="Notifications when task status changes">
        <input type="checkbox" checked={taskUpdates} onChange={(e) => { setTaskUpdates(e.target.checked); saveSetting('notify_task', e.target.checked) }} className="w-4 h-4 accent-white cursor-pointer" />
      </SettingRow>
      <SettingRow label="Agent Errors" description="Alert on agent execution errors">
        <input type="checkbox" checked={agentErrors} onChange={(e) => { setAgentErrors(e.target.checked); saveSetting('notify_error', e.target.checked) }} className="w-4 h-4 accent-white cursor-pointer" />
      </SettingRow>
      <SettingRow label="Deployment Complete" description="Notify when deployments finish">
        <input type="checkbox" checked={deployComplete} onChange={(e) => { setDeployComplete(e.target.checked); saveSetting('notify_deploy', e.target.checked) }} className="w-4 h-4 accent-white cursor-pointer" />
      </SettingRow>
    </Section>
  )
}
    ...
    <Section title="Keyboard Shortcuts" description="Customize keyboard shortcuts">
      {[
        ...
        <SettingRow key={s.action} label={s.action}>
          <kbd className="text-xs font-mono bg-[#080808] border border-[#202020] rounded px-2 py-1 text-[#6E6E6E]">{s.key}</kbd>
        </SettingRow>
      ))}
    </Section>
  )
...
const tabComponents: Record<string, React.FC> = {
  ...
  agents: () => <Section title="Agents" description="Agent settings can be configured from the Agents tab."><div className="p-4 text-sm text-[#6E6E6E]">Configure agents from the Agents page.</div></Section>,
