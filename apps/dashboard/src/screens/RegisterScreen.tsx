    ...
    <div className="min-h-screen bg-[#000000] flex relative overflow-hidden">
      ...
      >
        ...
        <div className="relative z-20 flex items-center gap-3 self-end mb-8">
          ...
          {/* Dark mode toggle */}
          <button
            onClick={() => { setDarkMode(!darkMode); localStorage.setItem('settings_theme', darkMode ? 'Light' : 'Dark'); }}
        ...
        <div className="relative z-10 flex flex-col justify-center flex-1">
          ...
          >
            Create applications, websites, automation, desktop software, mobile apps, and AI agents using natural language.
          </motion.p>
          <motion.div
            variants={stagger}
            initial="hidden"
        ...
        <div className="relative z-10 mt-12">
          <svg viewBox="0 0 400 60" className="w-full max-w-[300px] opacity-[0.04]" fill="none" stroke="#7C6BFF" strokeWidth="0.7">
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 lg:p-16 relative">
        ...
        >
          ...
          <div className="bg-[#111111] border border-[#202020] rounded-[22px] p-10 sm:p-12 shadow-[0_0_80px_rgba(0,0,0,0.5)]">
            ...
            <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.25 }} className="mb-8">
              <h2 className="text-[32px] sm:text-[36px] font-bold text-white tracking-[-0.02em] mb-2">Create account</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-3">
                <motion.div variants={inputVariant(0)} initial="hidden" animate="visible">
                  <div className="relative">
                    <Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B7280] pointer-events-none" />
                    <input
                      type="email"
                      value={email}
                ...
                <motion.div variants={inputVariant(1)} initial="hidden" animate="visible">
                  <div className="relative">
                    <Lock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B7280] pointer-events-none" />
                    <input
                    ...
                    />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#6B7280] hover:text-[#A1A1AA] transition-colors" aria-label={showPassword ? 'Hide password' : 'Show password'}>
              ...
              <AnimatePresence>{error && (
                <motion.p initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="text-xs text-[#EF4444] text-center">
                  {error}
                </motion.p>
              )}</AnimatePresence>
              <motion.div variants={inputVariant(3)} initial="hidden" animate="visible">
                <button type="submit" disabled={isLoading} className="w-full h-12 bg-white text-black text-sm font-semibold rounded-xl flex items-center justify-center gap-2.5 hover:bg-white/95 hover:shadow-[0_4px_20px_rgba(255,255,255,0.15)] hover:scale-[1.01] active:scale-[0.98] transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed">
                  {isLoading ? (
            ...
            <motion.div variants={inputVariant(4)} initial="hidden" animate="visible" className="flex items-center gap-4 my-7">
            ...
            <motion.div variants={inputVariant(5)} initial="hidden" animate="visible" className="flex gap-3">
              <button onClick={() => openOAuthPopup('github')} className="flex-1 h-11 flex items-center justify-center gap-2.5 rounded-xl border border-[#202020] bg-[#090909] text-[#A1A1AA] text-sm font-medium hover:bg-[#111] hover:border-[#333] hover:text-white transition-all duration-200">
              ...
              <button onClick={() => openOAuthPopup('google')} className="flex-1 h-11 flex items-center justify-center gap-2.5 rounded-xl border border-[#202020] bg-[#090909] text-[#A1A1AA] text-sm font-medium hover:bg-[#111] hover:border-[#333] hover:text-white transition-all duration-200">
